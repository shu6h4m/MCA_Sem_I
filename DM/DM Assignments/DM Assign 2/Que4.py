'''
Assignment 2nd
Q4) Write a program to implement the breadth first search(BFS) and depth first search(DFS) in graph.
'''
...
def bfs(graph_dict,startVertex):
    que=[startVertex]         #que initally containg starting vertex
    visited=[startVertex]       #list containg vistied node
    BFS=''                      #will contains BFS result

    #loop until que is not empty
    while len(que)!=0:
        current=que.pop(0)    #dequeue the que
...
def dfs(graph_dict,startVertex): #function to create DFS graph traversal
